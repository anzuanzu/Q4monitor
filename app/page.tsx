...
type Performance = { seasonJob: string; fundActual: string; insuranceActual: string };
...
function parseCsv(text: string) {
  ...
  const seasonIndex = getIndex('季職達', 'season_job_achievement'); const fundIndex = getIndex('第四季專案基金', 'q4_fund_actual'); const insuranceIndex = getIndex('第四季專案保險', 'q4_insurance_actual');
  ...
  return lines.slice(1).reduce<Record<string, Performance>>((records, line) => {
    ...
    records[key(cells[branchIndex], cells[nameIndex])] = { seasonJob: cells[seasonIndex] || '', fundActual: cells[fundIndex] || '', insuranceActual: cells[insuranceIndex] || '' };
    return records;
  }, {});
}
...
export default function Home() {
  ...
  return <main>
    <section className="hero"><div className="hero__topline"><span className="eyebrow">Q4 TARGETS · INTERNAL DASHBOARD</span><span className="data-status"><i />{source}</span></div><div className="hero__content"><div><p className="hero__kicker">第四季整合績效監控</p><h1>責任目標與實績</h1><p className="hero__description">基金與保險目標整合在同一張人員表；可直接載入含季職達、基金及保險數據的真實績效 CSV。</p></div><label className="upload-button">讀取績效 CSV<input type="file" accept=".csv,text/csv" onChange={handleFile} /></label></div></section>
    <section className="dashboard-shell">
      ...
      <section className="staff-section" aria-labelledby="staff-title"><div className="staff-section__header"><div><span className="eyebrow">INTEGRATED PERSONNEL TABLE</span><h2 id="staff-title">人員整合目標與實績</h2></div><span>{filtered.length} / {advisors.length} 筆</span></div><div className="filters"><label>分行<select value={branch} onChange={(event) => setBranch(event.target.value)}><option>全部分行</option>{branches.map((name) => <option key={name}>{name}</option>)}</select></label><label className="search-label">姓名<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="輸入姓名搜尋" /></label><label className="csv-tip">CSV 欄位：分行、理專姓名、季職達、第四季專案基金、第四季專案保險</label></div><div className="table-wrap"><table><thead><tr><th>分行</th><th>職級</th><th>理專姓名</th><th className="number">季職達</th><th className="number">第四季專案基金</th><th className="number">第四季專案保險</th><th className="number target-header">基金目標</th><th className="number target-header">保險目標</th></tr></thead><tbody>{filtered.map((advisor) => { const actual = performance[key(advisor.branch, advisor.name)]; return <tr key={key(advisor.branch, advisor.name)}><td>{advisor.branch}</td><td><span className="level-pill" style={{ color: colors[advisor.level] }}>{advisor.level}</span></td><td className="staff-name">{advisor.name}</td><td className="number actual-value">{actual?.seasonJob || '—'}</td><td className="number actual-value">{actual?.fundActual || '—'}</td><td className="number actual-value">{actual?.insuranceActual || '—'}</td><td className="number target-value">{fmt(advisor.fundTarget)} 萬</td><td className="number target-value">{fmt(insuranceTargets[advisor.level])} 萬</td></tr>; })}</tbody></table></div>{filtered.length === 0 && <p className="empty-state">找不到符合條件的人員。</p>}</section>
